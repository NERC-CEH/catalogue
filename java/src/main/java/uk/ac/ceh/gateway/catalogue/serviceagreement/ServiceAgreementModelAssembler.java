...
public class ServiceAgreementModelAssembler extends RepresentationModelAssemblerSupport<ServiceAgreement, ServiceAgreementModel> {
    ...
    protected ServiceAgreementModel instantiateModel(ServiceAgreement serviceAgreement) {
        ...
        val historyLink = linkTo(methodOn(ServiceAgreementController.class).getHistory(id))
                .withRel("history")
                .withTitle("History");
        ...
        if ("draft".equals(serviceAgreement.getState()) &&
            ...
            val submitLink = linkTo(methodOn(ServiceAgreementController.class).submitServiceAgreement(null, id))
                .withRel("submit")
                .withTitle("Submit");
        ...
        if ("pending publication".equals(serviceAgreement.getState())) {
            ...
            if (gemini.getState().equals("draft")) {
                val publishLink = linkTo(methodOn(ServiceAgreementController.class).publishServiceAgreement(null, id))
                    .withRel("publish")
                    .withTitle("Publish");
                model.add(publishLink);
                val addEdit = linkTo(methodOn(ServiceAgreementController.class).giveDepositorEditPermission(null, id))
                    .withRel("add-editor")
