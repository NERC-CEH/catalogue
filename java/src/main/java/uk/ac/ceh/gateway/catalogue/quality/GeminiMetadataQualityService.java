...
@ToString
@Service
public class GeminiMetadataQualityService implements MetadataQualityService {
    ...
    List<MetadataCheck> checkPointOfContact(DocumentContext parsed) {
        ...
        if (pocs.stream().anyMatch(poc -> fieldIsMissing(poc, "displayName") && (fieldIsMissing(poc, "familyName") || fieldIsMissing(poc, "givenName")))) {
            toReturn.add(new MetadataCheck("Point of contact name is missing or incomplete", ERROR));
        }
