...
class ServiceAgreementPublicationControllerTest {
    ...
    private void givenCurrentState() {
        val state = new StateResource(State.UNKNOWN_STATE, new HashSet<>(), file, catalogueKey, "service-agreement");
    ...
    void getCurrentPublication() {
        ...
        //when
        mvc.perform(
                get("/service-agreement/{file}/publication", file)
    ...
    void transitionState() {
        ...
        //when
        mvc.perform(
                post("/service-agreement/{file}/publication/{toState}", file, toState.getId())
