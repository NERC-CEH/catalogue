...
;// CONCATENATED MODULE: ./hubbub/src/Upload/DropzoneFileRow.tpl
// Module
var code = "<div class=\"uploading row file-row\"> <div class=\"col-md-7\"> <div class=\"row\"> <div class=\"col-md-6 file-name\"> <i class=\"fas fa-spinner fa-spin-2x\"></i> <span data-dz-name></span> </div> <div class=\"col-md-2 file-size\"> <span class=\"file-size-value\"></span> </div> <div class=\"col-md-4 file-status\">Uploading</div> <div class=\"col-md-12 file-message\"></div> </div> </div> <div class=\"col-md-5 file-actions\"> <a class=\"btn btn-danger cancel file-action\" role=\"button\"> <i class=\"fas fa-times\"></i> <span>Cancel</span> </a> </div> </div>";
// Exports
/* harmony default export */ const DropzoneFileRow = (code);
;// CONCATENATED MODULE: ./hubbub/src/Upload/DropzoneView.js





/* harmony default export */ const DropzoneView = (backbone_default().View.extend({
  initialize: function initialize(options) {
    var el = options.el,
        success = options.success,
        url = options.url; // eslint-disable-next-line no-new

    new $3ed269f2f0fb224b$export$2e2bcd8739ae039(el, this.dropzoneOptions(url, success));
  },
  dropzoneOptions: function dropzoneOptions(url, success) {
    return {
      timeout: -1,
      url: url,
      maxFilesize: 20 * 1000 * 1000,
      previewTemplate: DropzoneFileRow,
      ...
      init: function init() {
        this.on('addedfile', function (file) {
          ...
          var $file = jquery_default()(file.previewElement);
          $file.find('.cancel').click(function () {
            return _this.removeFile(file);
          });
          return $file.find('.file-size-value').text("".concat(filesize_min_default()(file.size)));
        });
        this.on('uploadprogress', function (file, progress, bytesSent) {
          var $file = jquery_default()(file.previewElement);

          if (progress < 100) {
            return $file.find('.file-status').text("Uploaded ".concat(filesize_min_default()(bytesSent)));
          } else {
        ...
        this.on('error', function (file, error, xhr) {
          ...
          if (xhr) {
            message = errorMessages[xhr.status] || error.error;
          }
...
;// CONCATENATED MODULE: ./hubbub/src/Upload/Upload.js

/* harmony default export */ const Upload = (backbone_default().Model.extend({
  urlRoot: '/upload'
}));
;// CONCATENATED MODULE: ./hubbub/src/File/File.js


var File = backbone_default().Model.extend({
  initialize: function initialize() {
    var action, classes, storage;
    ...
    var errorType = status in errorTypes ? errorTypes[status] : 'valid';
    var open = !validTypes.has(status);
    ...
    var size = this.has('bytes') ? filesize_min_default()(this.get('bytes')) : 0;
    var date = this.get('lastValidated');
    var hash = this.has('hash') ? this.get('hash') : 'NO_HASH';
    var message = messages[status];
    var moving = status === 'MOVING_FROM' || status === 'MOVING_TO' || status === 'WRITING';
    var validating = status === 'VALIDATING_HASH';
    return this.set({
      action: action,
      classes: classes,
      date: date,
  ...
  update: function update(data) {
    this.set({
      ...
      moving: false,
      path: data.path,
      size: filesize_min_default()(data.bytes),
      status: data.status
    });
    this.initialize();
  },
  copy: function copy(datastore) {
    return new File({
...
var validTypes = new Set(['MOVING_FROM', 'MOVING_TO', 'NO_HASH', 'VALID', 'VALIDATING_HASH', 'WRITING']);
...
;// CONCATENATED MODULE: ./hubbub/src/File/FileRow.tpl
// Module
var FileRow_code = "<div class=\"panel panel-file <%= classes %>\"> <div class=\"panel-heading\"> <% if (errorType === 'file') { %> <i class=\"file-icon fas fa-exclamation-circle\"></i> <% } else { %> <% if (status === 'MOVING_FROM' || status === 'MOVING_TO') { %> <span class=\"panel-heading-type\"><%= size %></span> <% } %> <% } %> <span><strong><%= status %></strong>&nbsp;</span> <span>&nbsp;<%= path %></span> <i class=\"panel-heading-chevron fas fa-chevron-down\"></i> </div> <div class=\"panel-body\"> <p> <b>Path: </b> <span><%= path %></span> </p> <div class=\"row\"> <div class=\"col-md-2\"><b>Status</b></div> <div class=\"col-md-2\"><b>Size</b></div> <div class=\"col-md-5\"><b>Checksum</b></div> <div class=\"col-md-3\"><b>Last validated</b></div> </div> <div class=\"row\"> <div class=\"col-md-2\"><%= status %></div> <div class=\"col-md-2\"><%= size %></div> <div class=\"col-md-5\"><%= hash %></div> <div class=\"col-md-3\"><%= date %></div> </div> <% if (message) { %> <div class=\"panel-file-message\"> <% if (message.title) { %> <span class=\"panel-file-message-title\"><%= message.title %></span> <% } %> <p class=\"panel-file-message-body\"><%= message.content %></p> </div> <% } %> </div> <div class=\"panel-footer\"> <div class=\"buttons\"> <% if (datastore === 'dropbox' && moving) { %> <button class=\"cancel btn btn-success\"> <i class=\"btn-icon fas fa-ban\"></i> <span>Cancel</span> </button> <% } else if (action === 'move-datastore') { %> <button class=\"move-datastore btn btn-success\"> <i class=\"btn-icon fas fa-level-down-alt\"></i> <span>Move to datastore</span> </button> <% } else if (action === 'move-metadata') { %> <button class=\"move-metadata btn btn-success\"> <i class=\"btn-icon fas fa-level-up-alt\"></i> <span>Move to metadata</span> </button> <% } else if (action === 'move-both') { %> <button class=\"delete btn btn-danger\"> <i class=\"btn-icon fas fa-trash\"></i> <span>Delete</span> </button> <button class=\"move-both move-metadata btn btn-success\"> <i class=\"btn-icon fas fa-level-down-alt\"></i> <span>Move to metadata</span> </button> <button class=\"move-both move-datastore btn btn-success\"> <i class=\"btn-icon fas fa-level-down-alt\"></i> <span>Move to datastore</span> </button> <% } else if (datastore === 'dropbox' && action === 'accept') { %> <button class=\"accept btn btn-primary\"> <i class=\"btn-icon fas fa-check\"></i> <span>Accept</span> </button> <% } else if (datastore === 'dropbox' && action === 'ignore') { %> <button class=\"ignore btn btn-danger\"> <i class=\"btn-icon fas fa-times\"></i> <span>Ignore</span> </button> <% }\n            if (datastore === 'dropbox') { %> <button class=\"delete btn btn-danger\"> <i class=\"btn-icon fas fa-trash\"></i> <span>Delete</span> </button> <% } %> <button class=\"validate btn btn-primary\"> <i class=\"btn-icon fas fa-check\"></i> <span>Validate</span> </button> </div> </div> </div> ";
// Exports
/* harmony default export */ const FileRow = (FileRow_code);
;// CONCATENATED MODULE: ./hubbub/src/File/FileView.js
...
/* harmony default export */ const FileView = (backbone_default().View.extend({
  ...
  initialize: function initialize(options) {
    this.template = index_default.template(FileRow);
  ...
  getServerState: function getServerState(self) {
    var timeout = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    var callback = arguments.length > 2 ? arguments[2] : undefined;
    setTimeout((jquery_default()).ajax, timeout, {
      url: "".concat(self.url, "/").concat(self.model.get('datastore'), "?path=").concat(encodeURIComponent(self.model.get('path'))),
      method: 'GET',
  ...
  accept: function accept(event) {
    ...
    var currentClasses = this.showInProgress(event);
    this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "/accept?path=").concat(encodeURIComponent(this.model.get('path'))), 'POST', function () {
      return _this.getServerState(_this, 3000, function () {
        return _this.showNormal(event, currentClasses);
      });
    });
  },
  cancel: function cancel(event) {
    ...
    if (window.confirm("Cancel moving file: ".concat(this.model.get('path'), "?"))) {
      this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "/cancel?path=").concat(encodeURIComponent(this.model.get('path'))), 'POST', function () {
        return _this2.getServerState(_this2, 3000, function () {
          return _this2.showNormal(event, currentClasses);
        });
      });
    } else {
  ...
  request: function request(self, event, url, method, success) {
    jquery_default().ajax({
      url: url,
      type: method,
      success: success,
  ...
  "delete": function _delete(event) {
    ...
    if (window.confirm("Delete file: ".concat(this.model.get('path'), "?"))) {
      this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "?path=").concat(encodeURIComponent(this.model.get('path'))), 'DELETE', function () {
        _this3.remove();

        _this3.collection.remove(_this3.model);
      });
    } else {
      this.showNormal(event, currentClasses);
    }
  },
  ignore: function ignore(event) {
    var _this4 = this;

    var currentClasses = this.showInProgress(event);

    if (window.confirm("Ignore file: ".concat(this.model.get('path'), "? This file will be unregistered from the system"))) {
      this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "/unregister?path=").concat(encodeURIComponent(this.model.get('path'))), 'POST', function () {
        _this4.remove();

        _this4.collection.remove(_this4.model);
      });
    } else {
      this.showNormal(event, currentClasses);
    }
  ...
  expand: function expand(event) {
    this.$(event.currentTarget).parent().toggleClass('is-collapsed');
  },
  move: function move(event, to, toCollection) {
    var _this5 = this;

    this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "/move?path=").concat(encodeURIComponent(this.model.get('path')), "&to=").concat(to), 'POST', function () {
      _this5.remove();

      _this5.collection.remove(_this5.model);

      var toModel = _this5.model.copy(to);

      toCollection.add(toModel);
    });
  },
  moveDatastore: function moveDatastore(event) {
    this.move(event, 'eidchub', this.datastore);
  },
  moveMetadata: function moveMetadata(event) {
  ...
  showInProgress: function showInProgress(event) {
    var $el = this.$(event.currentTarget);
    $el.attr('disabled', true);
    var $icon = $el.find('i');
    var current = $icon.attr('class');
    $icon.attr('class', 'btn-icon fas fa-sync fa-spin');
    return current;
  },
  showNormal: function showNormal(event, classes) {
    var $el = this.$(event.currentTarget);
    $el.attr('disabled', false);
    $el.find('i').attr('class', classes);
  },
  showInError: function showInError(event) {
    var $el = jquery_default()(event.currentTarget);
    $el.attr('disabled', true);
  ...
  validate: function validate(event) {
    ...
    var currentClasses = this.showInProgress(event);
    this.request(this, event, "".concat(this.url, "/").concat(this.model.get('datastore'), "/validate?path=").concat(encodeURIComponent(this.model.get('path'))), 'POST', function () {
      return _this6.getServerState(_this6, 3000, function () {
        return _this6.showNormal(event, currentClasses);
      });
    });
  }
}));
;// CONCATENATED MODULE: ./hubbub/src/File/FileCollection.js


/* harmony default export */ const FileCollection = (backbone_default().Collection.extend({
  model: File
}));
;// CONCATENATED MODULE: ./hubbub/src/File/index.js



;// CONCATENATED MODULE: ./hubbub/src/Upload/UploadView.js
...
/* harmony default export */ const UploadView = (backbone_default().View.extend({
  ...
  initialize: function initialize() {
    ...
    if (this.$('.dropzone-container').length) {
      var dropzoneSuccess = function dropzoneSuccess(file) {
        var filename = file.name.toLowerCase().replaceAll(' ', '-');
        var model = new File({
          bytes: file.size,
        ...
        _this.dropbox.add(model);

        jquery_default()(file.previewElement).remove();
    ...
    this.listenTo(this.datastore, 'reset', function (collection) {
      return _this.addAll(collection, _this.$datastore);
    });
    this.listenTo(this.datastore, 'add', function (model) {
      return _this.addOne(_this.datastore, _this.$datastore, model);
    });
    this.listenTo(this.datastore, 'update', function () {
      return _this.showEmptyStorage(_this.datastore, _this.$datastore, 'datastore');
    ...
    this.listenTo(this.dropbox, 'reset', function (collection) {
      return _this.addAll(collection, _this.$dropbox);
    });
    this.listenTo(this.dropbox, 'add', function (model) {
      return _this.addOne(_this.dropbox, _this.$dropbox, model);
    });
    this.listenTo(this.dropbox, 'update', function () {
      return _this.showEmptyStorage(_this.dropbox, _this.$dropbox, 'data');
    ...
    this.listenTo(this.metadata, 'reset', function (collection) {
      return _this.addAll(collection, _this.$metadata);
    });
    this.listenTo(this.metadata, 'add', function (model) {
      return _this.addOne(_this.metadata, _this.$metadata, model);
    });
    this.listenTo(this.metadata, 'update', function () {
      return _this.showEmptyStorage(_this.metadata, _this.$metadata, 'metadata');
    });
    var $datastoreData = jquery_default()('#datastore-data');

    if ($datastoreData.length) {
      var response = JSON.parse($datastoreData.text());
      this.model.set({
    ...
    var $dropboxData = jquery_default()('#dropbox-data');

    if ($dropboxData.length) {
      var _response = JSON.parse($dropboxData.text());

      this.model.set({
        dropboxPage: _response.meta.currentPage,
        dropboxSize: _response.meta.pageSize,
        dropboxLastPage: _response.meta.lastPage,
        dropboxTotalFiles: _response.meta.totalFiles
      });
      this.dropbox.reset(_response.data);
    }

    var $metadataData = jquery_default()('#metadata-data');

    if ($metadataData.length) {
      var _response2 = JSON.parse($metadataData.text());

      this.model.set({
        metadataPage: _response2.meta.currentPage,
        metadataSize: _response2.meta.pageSize,
        metadataLastPage: _response2.meta.lastPage,
        metadataTotalFiles: _response2.meta.totalFiles
      });
      this.metadata.reset(_response2.data);
    }
  },
  addOne: function addOne(collection, $container, model) {
  ...
  addAll: function addAll(collection, $container) {
    var _this2 = this;

    $container.empty();
    collection.each(function (model) {
      return _this2.addOne(collection, $container, model);
    });
  },
  loadMore: function loadMore(event, name, path, collection) {
    var _this3 = this;

    var nextPage = this.model.get("".concat(name, "Page")) + 1;
    var size = this.model.get("".concat(name, "Size"));
    var lastPage = this.model.get("".concat(name, "LastPage"));

    if (nextPage <= lastPage) {
      var currentClasses = this.showInProgress(event);
      return jquery_default().ajax({
        url: "".concat(this.model.url(), "/").concat(path, "?page=").concat(nextPage, "&size=").concat(size),
        success: function success(response) {
          _this3.showNormal(event, currentClasses);

          collection.add(response.data);

          _this3.model.set("".concat(name, "Page"), response.meta.currentPage);
        },
        error: function error() {
          return _this3.showInError(event);
        }
      });
    }
  },
  loadDatastore: function loadDatastore(event) {
    this.loadMore(event, 'datastore', 'eidchub', this.datastore);
  },
  loadDropbox: function loadDropbox(event) {
  ...
  renderDatastoreTotalFiles: function renderDatastoreTotalFiles() {
    var $totalFiles = this.$('.document-upload-section.datastore .total-files');
    var totalFiles = this.model.get('datastoreTotalFiles');
    console.log("rendering datastore to ".concat(totalFiles));

    if (totalFiles > 0) {
      $totalFiles.text("".concat(totalFiles, " files"));
    }
  },
  updateDatastoreTotalFiles: function updateDatastoreTotalFiles() {
    console.log("updating datastore to ".concat(this.datastore.length));
    this.model.set('datastoreTotalFiles', this.datastore.length);
  },
  renderDropboxTotalFiles: function renderDropboxTotalFiles() {
    var $totalFiles = this.$('.document-upload-section.dropbox .total-files');
    var totalFiles = this.model.get('dropboxTotalFiles');
    console.log("rendering dropbox to ".concat(totalFiles));

    if (totalFiles > 0) {
      $totalFiles.text("".concat(totalFiles, " files"));
    }
  },
  updateDropboxTotalFiles: function updateDropboxTotalFiles() {
    console.log("updating dropbox to ".concat(this.dropbox.length));
    this.model.set('dropboxTotalFiles', this.dropbox.length);
  },
  showEmptyStorage: function showEmptyStorage(collection, $container, title) {
    if (collection.length === 0) {
      $container.append("<h3 class=\"no-documents text-center\">NO FILES IN ".concat(title.toUpperCase(), "</h3>"));
    } else {
  ...
  finish: function finish(event) {
    ...
    var currentClasses = this.showInProgress(event);
    return jquery_default().ajax({
      url: "".concat(this.model.url(), "/finish"),
      type: 'POST',
  ...
  redirectToDocuments: function redirectToDocuments() {
    window.location.assign("/documents/".concat(this.model.get('id')));
  },
  hashDropbox: function hashDropbox(event) {
    var that = this;
    var currentClasses = this.showInProgress(event);
    return jquery_default().ajax({
      url: "".concat(this.model.url(), "/hash"),
      type: 'POST',
      success: function success() {
  ...
  moveAllDatastore: function moveAllDatastore(event) {
    var that = this;
    var currentClasses = this.showInProgress(event);
    jquery_default().ajax({
      url: "".concat(this.model.url(), "/dropbox/move?to=eidchub"),
      type: 'POST',
      success: function success() {
        that.dropbox.each(function (model) {
          return that.addOne(that.datastore, that.$datastore, model.copy('eidchub'));
        });
        that.dropbox.reset();
        that.showNormal(event, currentClasses);
      },
  ...
  registerDropbox: function registerDropbox(event) {
    var that = this;
    var currentClasses = this.showInProgress(event);
    return jquery_default().ajax({
      url: "".concat(this.model.url(), "/register"),
      type: 'POST',
      success: function success() {
        that.showNormal(event, currentClasses);
        that.reloadPage();
  ...
  reschedule: function reschedule(event) {
    var that = this;
    var currentClasses = this.showInProgress(event);
    jquery_default().ajax({
      url: "".concat(this.model.url(), "/reschedule"),
      type: 'POST',
      success: function success() {
        that.showNormal(event, currentClasses);
        that.reloadPage();
  ...
  schedule: function schedule(event) {
    ...
    var currentClasses = this.showInProgress(event);
    return jquery_default().ajax({
      url: "".concat(this.model.url(), "/schedule"),
      type: 'POST',
      success: function success() {
        that.showNormal(event, currentClasses);
        that.reloadPage();
  ...
  validateAll: function validateAll(event) {
    var currentClasses = this.showInProgress(event);
  ...
  validate: function validate(event, name, datastore, collection, currentClasses) {
    var _this4 = this;

    jquery_default().ajax({
      url: "".concat(this.model.url(), "/").concat(datastore, "/validate"),
      type: 'POST',
      success: function success() {
        return _this4.collectionSuccess(event, currentClasses, name, datastore, collection);
      },
      error: function error(xhr) {
        if (xhr.status !== 400) {
          _this4.showInError(event);
        }
      }
    });
  },
  collectionSuccess: function collectionSuccess(event, currentClasses, name, datastore, collection) {
    var _this5 = this;

    setTimeout(function () {
      return _this5.getServerState(name, datastore, collection, function () {
        return _this5.showNormal(event, currentClasses);
      });
    }, 7000);
  },
  getServerState: function getServerState(name, datastore, collection, callback) {
    var page = 1;
    var collectionSize = collection.length;
    var size = this.model.has("".concat(name, "Size")) ? this.model.get("".concat(name, "Size")) : 20;

    if (collectionSize > size) {
      size = collectionSize;
    }

    this.model.set("".concat(name, "Page"), page);
    this.model.set("".concat(name, "Size"), size);
    jquery_default().ajax({
      url: "".concat(this.model.url(), "/").concat(datastore, "?page=").concat(page, "&size=").concat(size),
      dataType: 'json',
      success: function success(response) {
...
;// CONCATENATED MODULE: ./hubbub/src/Upload/index.js



;// CONCATENATED MODULE: ./hubbub/src/index.js


function init() {
  var id = jquery_default()('#document-upload').data('guid');
...
jquery_default()(document).ready(init);
})();
