...
;// CONCATENATED MODULE: ./search/src/models/SearchPage.js


/* harmony default export */ const SearchPage = (backbone_default().Model.extend({
  url: function url() {
  ...
  */
  getSelectedResult: function getSelectedResult() {
    ...
    if (this.has('results')) {
      index_default.find(this.attributes.results, function (e) {
        return e.identifier === _this.attributes.selected;
      });
    }
  }
}));
;// CONCATENATED MODULE: ./search/src/SearchApp.js
...
/* harmony default export */ const SearchApp = (backbone_default().Model.extend({
  defaults: {
    drawing: false,
    mapsearch: false,
    bbox: undefined,
  ...
  */
  ...
  propergate out.
    ...
    */
  createSearchPage: function createSearchPage() {
    var _this = this;

    this._proxyResultsEvents = function (evt) {
      return _this.trigger("results-".concat(evt));
    };
  ...
  */
  jumpToPageOne: function jumpToPageOne(evt) {
    if (!evt.changed.page) {
  ...
  */
  disableDrawing: function disableDrawing(evt) {
    if (!evt.changed.drawing) {
  ...
  */
  performSearch: function performSearch(evt) {
    var _$chain;

    if (!(_$chain = index_default.chain(evt.changed)).pick.apply(_$chain, _toConsumableArray(Array.from(this.searchFields || []))).isEmpty().value()) {
  ...
  */
  getState: function getState() {
    var state = this.pick.apply(this, _toConsumableArray(Array.from(this.searchFields || [])));

    index_default.each(state, function (v, k) {
      if (!v) {
  ...
  */
  setState: function setState(state, options) {
    var searchDefaults = index_default.pick.apply(index_default, [this.defaults].concat(_toConsumableArray(Array.from(this.searchFields))));

    this.set(index_default.defaults(state, searchDefaults), options);
  },
...
;// CONCATENATED MODULE: ./node_modules/deparam.js/dist/esm/deparam.esm.js
/*!
...
var deparam_esm_isNumber = function(num) {
...
var deparam_esm_isObject = function isObject(x) {
...
function resolveObj(ob, nextProp) {
  if (deparam_esm_isObject(ob) && !isArr(ob)) return {
    ob: ob
  };
  if (isArr(ob) || _typeof(ob) === TYPEOF_UNDEF) return {
    ob: resolve(ob, deparam_esm_isNumber(nextProp))
  };
  return {
    ob: [ob],
    push: ob !== null
...
;// CONCATENATED MODULE: ./search/src/templates/FacetsPanel.tpl
// Module
var code = "<% /*\r\n This underscore template generates a facet results set with a heading\r\n\r\n IMPORTANT: If you change the structure of this, please update the \r\n corresponding freemarker template /templates/search/_facets.tpl\r\n*/ %> <% _.each(facets, function(facet) { %> <h3><%= facet.displayName %></h3> <%= template({\"results\": facet.results, \"template\": template}) %> <% }); %> ";
// Exports
/* harmony default export */ const FacetsPanel = (code);
;// CONCATENATED MODULE: ./search/src/templates/FacetResults.tpl
// Module
var FacetResults_code = "<% /*\r\n This underscore template generates a given facet results client side. \r\n It is recusively defined and as such needs to be supplied with itself\r\n\r\n IMPORTANT: If you change the structure of this, please update the \r\n corresponding freemarker template /templates/search/_facets.tpl\r\n*/ %> <ul> <% _.each(results, function(facet) { %> <li> <a href=\"<%=facet.url%>\" title=\"<%=facet.name%> (<%=facet.count%>)\"><%=facet.name%> <small class=\"facet-count\">(<%=facet.count%>)</small></a> <% if(facet.active) { %> <a href=\"<%=facet.url%>\" title=\"<%=facet.name%> (<%=facet.count%>)\"> <span class=\"fas fa-times\"></span> </a> <% } %> <% if (facet.subFacetResults) { %> <%= template({results:facet.subFacetResults, template: template}) %> <% } %> </li> <% }); %> </ul>";
// Exports
/* harmony default export */ const FacetResults = (FacetResults_code);
;// CONCATENATED MODULE: ./search/src/views/FacetsPanelView.js




/* harmony default export */ const FacetsPanelView = (backbone_default().View.extend({
  initialize: function initialize() {
    this.panelTpl = index_default.template(FacetsPanel);
    this.resultsTpl = index_default.template(FacetResults);
...
;// CONCATENATED MODULE: ./search/src/views/SearchFormView.js


/* harmony default export */ const SearchFormView = (backbone_default().View.extend({
  events: {
  ...
  */
  updateTermOnModel: function updateTermOnModel() {
  ...
  */
  getDisplayedTerm: function getDisplayedTerm() {
  ...
  */
  updateDisplayedTerm: function updateDisplayedTerm() {
    var term = this.model.get('term');
    var displayed = jquery_default()("[name='term']").val();

    if (term !== displayed) {
      jquery_default()("[name='term']").val(term);
    }
  }
}));
;// CONCATENATED MODULE: ./search/src/templates/SearchPage.tpl
// Module
var SearchPage_code = "<% /*\r\n This underscore template generates the search results client side.\r\n\r\n IMPORTANT: If you change the structure of this, please update the\r\n corresponding freemarker template /templates/search/_page.ftl\r\n*/ %> <div class=\"results__header\"> <span id=\"num-records\"><%=numFound%></span> records found </div> <div class=\"results__related_searches\"/> <div class=\"results__list\"> <% _.each(results, function(result) { %> <a class=\"result result--<%=result.state%> <% if (result.resourceStatus != '') { %>result--<%=result.resourceStatus%><% } %>\" id=\"<%=result.identifier%>\" href=\"/documents/<%=result.identifier%>\"> <div class=\"result__tags\"> <div class=\"recordType\"> <% if (result.documentType != '' && result.documentType == \"LINK_DOCUMENT\") {  %> <i class=\"fas fa-link\"></i> Linked <% } %> <span><%=result.recordType%></span> </div> <% if (result.resourceStatus != '') {  %> <div class=\"resourceStatus\"><%=result.resourceStatus%></div> <% } %> <div class=\"state\"> <% if(result.state == 'draft') { %> <span class=\"text-draft\"><b>DRAFT</b></span> <% } else if(result.state == 'pending') { %> <span class=\"text-pending\"><b>PENDING PUBLICATION</b></span> <% } %> </div> <% if (typeof result.condition != \"undefined\" && result.condition != '') {  %> <div class=\"condition\"><%=result.condition%></div> <% } %> </div> <div class=\"result__title\"><%=result.title%></div> <div class=\"result__description\"><%=result.shortenedDescription%></div> <% if(result.incomingCitationCount != 0) { %> <div class=\"result__citationCount\"><%=result.incomingCitationCount%> citation<% if(result.incomingCitationCount >1) { %>s<% } %></div> <% } %> </a> <% }); %> </div> <div class=\"results__footer\"> <ul class=\"pager\"> <% if(prevPage) { %> <li class=\"previous\"><a href=\"<%=prevPage%>\">&larr; Previous</a></li> <% } %> <li>Page <%=page%></li> <% if(nextPage) { %> <li class=\"next\"><a href=\"<%=nextPage%>\">Next &rarr;</a></li> <% } %> </ul> </div> ";
// Exports
/* harmony default export */ const templates_SearchPage = (SearchPage_code);
;// CONCATENATED MODULE: ./search/src/views/SearchPageView.js




/* harmony default export */ const SearchPageView = (backbone_default().View.extend({
  initialize: function initialize() {
    this.template = index_default.template(templates_SearchPage);
    ...
    index_default.bindAll(this, 'padResults');

    jquery_default()(window).on('resize', this.padResults);
  },
  ...
  */
  padResults: function padResults() {
    if (this.$('.result').length) {
      // Where the top result should start
      var topPosition = jquery_default()('.navbar').height() + jquery_default()('.search-form').height();
      var onScreen = this.$('.result').last().nextAll().addBack(); // Elements to show

      var onScreenHeight = index_default.reduce(onScreen, function (height, e) {
        return height + jquery_default()(e).outerHeight(true);
      }, topPosition);

      this.$el.css({
        marginBottom: jquery_default()(window).height() - onScreenHeight
      });
    }
  },
  ...
  */
  readModelFromHTML: function readModelFromHTML() {
    this.model.getResults().set({
      numFound: this.$('#num-records').val(),
      results: index_default.map(this.$('.result'), function (r) {
        return {
          identifier: jquery_default()(r).attr('id'),
          title: jquery_default()('.result__title', r).text(),
          description: jquery_default()('.result__description', r).text(),
          locations: jquery_default()(r).attr('data-location').split('|')
        };
      })
    });
  },
  ...
  */
  updateSelected: function updateSelected() {
    var selected = this.model.getResults().get('selected');
    this.$('.result').removeClass('selected');
    this.$("#".concat(selected)).addClass('selected');
  },
  ...
  */
  render: function render() {
    ...
    var $relatedSearches = this.$('.results__related_searches');
    var relatedSearches = this.model.getResults().get('relatedSearches');
    ...
    if (relatedSearches != null) {
      ...
      relatedSearches.forEach(function (relatedSearch, index) {
        ...
        $relatedSearches.append("".concat(prefix, "<a href=\"").concat(relatedSearch.href, "\">").concat(relatedSearch.title, "</a>"));
      });
    }
...
;// CONCATENATED MODULE: ./search/src/templates/Message.tpl
// Module
var Message_code = "<div class=\"alert alert-<%=type%>\" role=\"alert\"> <span class=\"fas fa-times\"></span> <span><%=message%></span> </div>";
// Exports
/* harmony default export */ const Message = (Message_code);
;// CONCATENATED MODULE: ./search/src/views/MessageView.js




/* harmony default export */ const MessageView = (backbone_default().View.extend({
  el: '#message-panel',
  ...
  initialize: function initialize() {
    this.template = index_default.template(Message);
  ...
  /*
    ...
    */
  appendError: function appendError(model, res) {
    var message;
    this.$el.show();

    if (index_default.isString(model)) {
      message = model;
    }

    this.$el.append(this.template({
      type: 'danger',
      message: message || __guard__(res != null ? res.responseJSON : undefined, function (x) {
        return x.message;
      }) || this.defaultMessage
    }));
  },
  ...
  /*
    ...
    */
  appendInfo: function appendInfo(message) {
    this.$el.show();
    this.$el.append(this.template({
      type: 'info',
      message: message
  ...
  dismissMessage: function dismissMessage(evt) {
    jquery_default()(evt.target).parent('.alert').remove();
...
;// CONCATENATED MODULE: ./search/src/views/index.js




;// CONCATENATED MODULE: ./search/src/SearchAppView.js




/* harmony default export */ const SearchAppView = (backbone_default().View.extend({
  el: '#search',
  events: {},
  initialize: function initialize(options) {
    this.appUrl = window.location.href.split('#')[0].split('?')[0]; // Mutate the events hash so that is listens to clicks of urls which will
  ...
  */
  handleUrl: function handleUrl(e) {
    var query = jquery_default()(e.currentTarget).attr('href').split('?')[1];
    this.model.setState(lib(query, true));
    e.preventDefault();
  },
  ...
  the default state.
  */
  defaultState: function defaultState(e) {
    this.model.setState({});
    e.preventDefault();
  },
...
;// CONCATENATED MODULE: ./search/src/SearchRouter.js



/* harmony default export */ const SearchRouter = (backbone_default().Router.extend({
  routes: {
    '*data': 'updateModel'
  },
  initialize: function initialize(options) {
    ...
    this.model = options.model; // If there is no hash component, we can use the query string to update the
    // model to represent the state of the document which is already loaded

    if (!options.location.hash) {
      this.updateModel(options.location.search.substring(1), {
        silent: true
      });
    }
  ...
  updateRoute: function updateRoute() {
    var queryString = jquery_default().param(this.model.getState(), true);
    return this.navigate(queryString, {
      replace: true
    });
  },
  ...
  /*
    ...
    */
  updateModel: function updateModel(state, options) {
    if (state) {
      return this.model.setState(lib(state, true), options);
    }
  }
}));
;// CONCATENATED MODULE: ./search/src/index.js
...
jquery_default()(document).ready(function () {
  if (jquery_default()('#search').length) {
    /*
      Initialize the search
      */
    var app = new SearchApp();
    var view = new SearchAppView({
      model: app
    });
