...
;// CONCATENATED MODULE: ./permission/src/IdentityPermission/IdentityPermission.js

/* harmony default export */ const IdentityPermission = (backbone_default().Model.extend({
...
;// CONCATENATED MODULE: ./permission/src/IdentityPermission/IdentityPermissions.js


/* harmony default export */ const IdentityPermissions = (backbone_default().Collection.extend({
  model: IdentityPermission,
  comparator: 'identity'
}));
;// CONCATENATED MODULE: ./permission/src/IdentityPermission/IdentityPermission.tpl
// Module
var code = "<td><%= identity %></td> <td><input data-permission=\"canView\" type=\"checkbox\" <% if(canView) { %> checked=\"checked\" <% } %>></td> <td><input data-permission=\"canEdit\" type=\"checkbox\" <% if(canEdit) { %> checked=\"checked\" <% } %>></td> <td><input data-permission=\"canDelete\" type=\"checkbox\" <% if(canDelete) { %> checked=\"checked\" <% } %>></td> <td><input data-permission=\"canUpload\" type=\"checkbox\" <% if(canUpload) { %> checked=\"checked\" <% } %>></td> <td><button class=\"editor-button-xs\"><i class=\"fas fa-times\"></i></button></td>";
// Exports
/* harmony default export */ const IdentityPermission_IdentityPermission = (code);
// EXTERNAL MODULE: ./node_modules/jquery/dist/jquery.js
var jquery = __webpack_require__(9755);
var jquery_default = /*#__PURE__*/__webpack_require__.n(jquery);
;// CONCATENATED MODULE: ./permission/src/IdentityPermission/IdentityPermissionView.js




/* harmony default export */ const IdentityPermissionView = (backbone_default().View.extend({
  tagName: 'tr',
  events: {
  ...
  initialize: function initialize() {
    this.template = index_default.template(IdentityPermission_IdentityPermission);
  },
  removePermission: function removePermission() {
    this.model.parent.removePermission(this.model);
  },
  update: function update(event) {
    var permission = jquery_default()(event.target).data('permission');
...
;// CONCATENATED MODULE: ./permission/src/IdentityPermission/index.js



;// CONCATENATED MODULE: ./permission/src/PermissionApp/Permission.js



/* harmony default export */ const Permission = (backbone_default().Model.extend({
  url: function url() {
    return this.urlRoot();
  },
  urlRoot: function urlRoot() {
    return "/documents/".concat(this.id, "/permission");
  },
  loadCollection: function loadCollection() {
    var collection = new IdentityPermissions();
    collection.reset(this.get('permissions'));
    this.set('permissions', collection);
  },
  addPermission: function addPermission(permission) {
    var collection = index_default.clone(this.get('permissions'));
...
;// CONCATENATED MODULE: ./permission/src/PermissionApp/PermissionApp.js



/* harmony default export */ const PermissionApp = (backbone_default().Model.extend({
  loadPermission: function loadPermission(identifier) {
    var _this = this;
    ...
    return permission.fetch({
      success: function success(model) {
        ...
        _this.set('permission', model);

        _this.trigger('loaded');
      },
      error: function error(model) {
        _this.trigger('error', "Unable to load permission for: ".concat(model.id));
      }
    });
  },
  getPermission: function getPermission() {
    return index_default.clone(this.get('permission'));
  }
}));
;// CONCATENATED MODULE: ./permission/src/PermissionApp/Permissions.tpl
// Module
var Permissions_code = "<h2>Permissions</h2> <div> <p><i class=\"fas fa-info-circle fa-2x pull-right\"></i> Amend permissions for users or groups.. For external users, the username is the email address with which they registered an account.<br>For UKCEH staff it is their login username <b>not</b> their email address.</p> <p><b>NOTE</b>: Catalogue administrators have permission to add and edit all records regardless of permissions defined here.</p> </div> <table class=\"table table-striped\"> <thead> <tr> <th>Username/Group</th> <th>Can view</th> <th>Can edit</th> <th>Can delete</th> <th>Can upload</th> <th>Action</th> </tr> </thead> <tfoot> <tr> <td><input id=\"identity\" class=\"form-control\" placeholder=\"username/group\"></td> <td><input id=\"canView\" type=\"checkbox\"></td> <td><input id=\"canEdit\" type=\"checkbox\"></td> <td><input id=\"canDelete\" type=\"checkbox\"></td> <td><input id=\"canUpload\" type=\"checkbox\"></td> <td><button id=\"permissionAdd\" class=\"editor-button-xs\" title=\"remove permissions for this user\">Add</button></td> </tr> </tfoot> <tbody> </tbody> </table> <div class=\"navbar navbar-default navbar-fixed-bottom\"> <div class=\"container\"> <div class=\"navbar-right\"> <a href=\"/documents/<%= id %>/permission\" class=\"btn btn-default navbar-btn\">Cancel</a> <button id=\"permissionSave\" class=\"btn btn-primary navbar-btn\"><i class=\"far fa-save\"></i> Save</button> </div> </div> </div>";
// Exports
/* harmony default export */ const Permissions = (Permissions_code);
;// CONCATENATED MODULE: ./permission/src/PermissionApp/PermissionView.js





/* harmony default export */ const PermissionView = (backbone_default().View.extend({
  el: '.permission',
  events: {
  ...
  initialize: function initialize() {
    this.template = index_default.template(Permissions);
  ...
  save: function save() {
    ...
    this.model.save({}, {
      success: function success() {
        _this.model.trigger('save:success', 'Save successful');
      },
      error: function error(model, response) {
        _this.model.trigger('save:error', "Error saving permission: ".concat(response.status, " (").concat(response.statusText, ")"));
      }
    });
  },
  addOne: function addOne(permission) {
    index_default.extend(permission, {
      parent: this.model
    });

    var view = new IdentityPermissionView({
      model: permission
    });
  ...
  add: function add() {
    var identity = jquery_default()('#identity').val();

    if (identity) {
      var permission = new IdentityPermission({
        identity: identity,
        canView: jquery_default()('#canView').prop('checked'),
        canEdit: jquery_default()('#canEdit').prop('checked'),
        canDelete: jquery_default()('#canDelete').prop('checked'),
        canUpload: jquery_default()('#canUpload').prop('checked')
      });
      this.model.addPermission(permission);
    }

    jquery_default()('#identity').val('');
    jquery_default()('#canView').prop('checked', false);
    jquery_default()('#canEdit').prop('checked', false);
    jquery_default()('#canDelete').prop('checked', false);
    jquery_default()('#canUpload').prop('checked', false);
  },
  leave: function leave() {
    window.location.assign("/documents/".concat(this.model.get('id'), "/permission"));
  }
}));
;// CONCATENATED MODULE: ./permission/src/PermissionApp/PermissionAppView.js


/* harmony default export */ const PermissionAppView = (backbone_default().View.extend({
  el: '.permission',
  initialize: function initialize() {
...
;// CONCATENATED MODULE: ./permission/src/PermissionApp/index.js




;// CONCATENATED MODULE: ./permission/src/index.js


jquery_default()(document).ready(function () {
  var app = new PermissionApp();
  app.loadPermission(jquery_default()('#metadata').data('id'));
  var view = new PermissionAppView({
    model: app
  });
});
})();
