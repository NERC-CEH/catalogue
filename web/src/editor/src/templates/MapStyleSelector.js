...
<div id="map-style-selector" class="input-group">
  <input id="color_code" type="text" class="form-control" value="<%= data.type %>" <%= data.disabled%>/>
  <div class="input-group-btn">
    ...
    <ul class="dropdown-menu dropdown-menu-end">
      ...
      <% _.chain(data.symbols).each(function(s, id){%>
        <li><a class="dropdown-item" href="#" data-symbol="<%=id%>"><span style="margin-right: 0.5em;"><%=s.icon%></span><%=s.label%></a></li>
      <%})%>
    </ul>
  </div>
</div>
