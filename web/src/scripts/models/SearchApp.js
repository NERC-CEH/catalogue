...
define([
...
], function(_, Backbone, SearchPage) { return Backbone.Model.extend({
  ...
  */
  ...
  initialize() {
    ...
    return this.on('change', this.performSearch);
  },
  ...
  */
  jumpToPageOne(evt){ if (!evt.changed.page) { return this.set('page', this.defaults.page); } },
  ...
  */
  disableDrawing(evt) { if (!evt.changed.drawing) { return this.set('drawing', false); } },
  ...
  */
  performSearch(evt) {
    if (!_.chain(evt.changed).pick(...Array.from(this.searchFields || [])).isEmpty().value()) {
      ...
      return this.results.fetch({cache: false, traditional:true, data: this.getState()});
    }
  },
  ...
  */
  setState(state, options) {
    ...
    return this.set(_.defaults(state, searchDefaults), options);
  },
  ...
  */
  clearResults() { 
    ...
    this.results = null;
    return this.trigger('cleared:results');
  }
});
 });